...
#[test]
fn test_accept_offer_apollo42_series() {
    let (marketplace, nft, _, alice, bob, chandra, darmaji, _) = init();
    ...
    // chandra accept bid and nft_approve to marketplace
    let msg = &json!(
        {"market_type":"accept_offer_apollo42_series","buyer_id": bob.account_id, "price": U128(10u128.pow(24))}).to_string();
...
#[test]
fn test_accept_trade_apollo42_series(){
    let (marketplace, nft, _, alice, bob, chandra, darmaji, _) = init();
    create_nft_and_mint_one(&nft, &alice, &bob, &chandra, &darmaji);
    ...
    darmaji.call(
        ...
        &json!({
            ...
            "account_id": marketplace.account_id(),
            "msg": &json!{{
                "market_type": "accept_trade_apollo42_series",
                "buyer_id": chandra.account_id(),
                "buyer_nft_contract_id": nft.account_id(),
                "buyer_token_id": "1:1"
            }}.to_string()
